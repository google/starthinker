...
from util.dbm import accounts_split


def get_dbm_query(dbm_accounts):
  partners, advertisers = accounts_split(dbm_accounts)
  query = 'SELECT * FROM [Brand_Safety.Master_Transform] WHERE Partner_ID IN (%s)' % ','.join(map(lambda s:str(s), partners))
  if advertisers: query += ' AND Advertiser_ID IN (%s)' % ','.join(map(lambda s:str(s), advertisers)) 
  return query
...
def brand():
  ...
  if project.task.get('dbm_accounts', None):
    query_to_table(project.task['auth'], project.id, project.task['dataset'], 'Brand_Safety_DBM', get_dbm_query(project.task['dbm_accounts']))
