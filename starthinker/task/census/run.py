...
def census_correlate(column_join, column_pass, column_sum, column_correlate, dataset, table):
...
def census_significance(column_join, column_pass, column_sum, column_correlate, dataset, significance):
  ...
  for segment in CENSUS_FIELDS:
    if segment['category']:
      for column_a in segment['columns']:
        for column_b in column_correlate:
          query += '%s.pearson_significance_test(%s_%s_Correlation, Locations, %s, 0.5) AS %s_%s_Correlation,\n' % (
            dataset,
...
def census_pivot(column_join, column_pass, column_sum, column_correlate, dataset):
  ...
  for segment in CENSUS_FIELDS:
    if segment['category']:
      for column_a in segment['columns']:
        query += '    STRUCT(\n'
        for column_b in column_correlate:
          query += '      %s_%s_Correlation AS %s,\n' % (column_a.title(), column_b, column_b)
