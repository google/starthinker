...
def dv_insertion_order_clear(config, task):
  table_create(
    config,
    task["auth_bigquery"],
    config.project,
...
def dv_insertion_order_load(config, task):
  ...
  put_rows(
    ...
    dv_insertion_order_load_multiple()
  )
...
def dv_insertion_order_audit(config, task):
...
def dv_insertion_order_patch(commit=False):
...
def dv_insertion_order_insert(commit=False):
  inserts = []
...
def dv_insertion_order_commit(patches):
