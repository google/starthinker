...
def get_changelogs(accounts, start):
  ...
  if project.verbose: print('CM CHANGE LOGS', accounts)
...
def dcm_log():
  if project.verbose: print('DCM LOG')
  ...
  if table_exists(
    ...
    start = next(query_to_rows(
      project.task['out']['auth'],
      project.task['out']['project'],
      project.task['out']['dataset'],
      'SELECT FORMAT_TIMESTAMP("%%Y-%%m-%%dT%%H:%%M:%%S-00:00", MAX(changeTime), "UTC") FROM `%s`' % CHANGELOGS_TABLE,
      1,
      False
    ))[0]
    ...
    start = (datetime.utcnow() - timedelta(days=int(project.task['days']))).strftime('%Y-%m-%dT%H:%M:%S-00:00')
