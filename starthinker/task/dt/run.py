...
def dt_timestamp(dt_filepath):
  ...
  if parts:
    ...
    if len(dt_time) == 10: dt_time = project.timezone.localize(datetime.strptime(dt_time, '%Y%m%d%H'))
    else: dt_time = project.timezone.localize(datetime.strptime(dt_time, '%Y%m%d'))
