...
def sheets_id(auth, url_or_name):
  ...
  if url_or_name.startswith('https://docs.google.com/spreadsheets/d/'):
    m = re.search('^(?:https:\/\/docs.google.com\/spreadsheets\/d\/)?([a-zA-Z0-9-_]+)(?:\/.*)?$', url_or_name)
...
def sheets_tab_copy(auth, from_sheet_url_or_name, from_sheet_tab, to_sheet_url_or_name, to_sheet_tab, overwrite=False):
  ...
  if overwrite or to_tab_id is None:
    ...
    API_Retry(service.spreadsheets().batchUpdate(spreadsheetId=to_sheet_id, body=body))
...
def sheets_create(auth, sheet_name, sheet_tab, template_sheet=None, template_tab=None):
  ...
  if not, creates the sheet. Both sheet and tab must be provided or both must be omitted to create
  a blank sheet and tab.

  Args:
    * auth: (string) Either user or service.
    * sheet_name: (string) name of sheet to create, used as key to check if it exists in the future.
