...
class FieldMap:
  ...
  PLACEMENT_PERIOD_END = 'Pacing Period End Date'
...
class Feed:
  ...
  _feed_name_tab_map = {
      ...
      'creative_asset_association_feed': ['Asset Creative Assignment', 'Creative'],
      'creative_campaign_association_feed': ['Campaign Creative Assignment', 'Creative'],
      'transcode_configs_feed': ['Transcode Configuration'],
      'event_tag_profile_feed': ['Event Tag Profile'],
      'lp_dawn': ['Form Responses 1']
  }

  def __init__(self, auth, trix_id, feed_name, parse=True):
    ...
    self.feed = self._feed_to_dict(self._get_feed(), parse=self._parse)

  def update(self):
    """Updates the related Bulkdozer feed item with the values in this object.

    """
    new_feed = self._dict_to_feed(self.feed, parse=self._parse)
  ...
  def _dict_to_feed(self, feed, parse=True):
    ...
    for item in feed:
      for key in item.keys():
        if key in headers:
          ...
          while column >= len(raw_feed[row]):
            raw_feed[row].append('')

          raw_feed[row][column] = value

      row += 1

    return raw_feed

  def _feed_to_dict(self, raw_feed, parse=True):
