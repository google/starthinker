...
class Account(AbstractBaseUser):
  ...
  def set_credentials(self, credentials):
    ...
    if self.identifier: BucketCredentials.from_oauth(self.get_credentials_safe(), credentials).to_bucket()

  def get_credentials(self):
    return BucketCredentials.from_bucket(self.get_credentials_safe()) if self.identifier else None

  def get_credentials_safe(self):
    return '%s:ui/%s.json' % (settings.UI_BUCKET_AUTH, self.identifier)

  def get_bucket(self, full_path=True):
    bucket = 'starthinker-account-%d' % self.id
