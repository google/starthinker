...
from starthinker.ui.recipe.forms_fields import CommaSeparatedCharField, CommaSeparatedIntegerField, JsonField, TimezoneField
...
class ScriptJsonForm(forms.Form):
  ...
  def __init__(self, sequence, script, values, constants, *args, **kwargs):
    self.script = script
    self.constants = constants
    super(ScriptJsonForm, self).__init__(*args, **kwargs)
    self.fields['script_sequence'].initial = sequence
    x = script.get_tag()
    self.variables = json_get_fields(script.get_task())
    for variable in self.variables:
      if variable['name'] in constants: continue # skip inputs that come from recipe constants
  ...
  def get_description(self):
    return self.script.get_description(self.constants)

  def get_instructions(self):
    return self.script.get_instructions(self.constants)
  ...
  def get_script(self):
    ...
    # if saving, return values ( constants are not stored as they come from the recipe )
      ...
      values = {}
      for variable in self.variables: 
        if variable['name'] in self.constants: continue # skip inputs that come from recipe constants
