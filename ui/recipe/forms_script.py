...
from starthinker.ui.recipe.forms import SetupForm
...
class ScriptForm(forms.Form):
  ...
  def __init__(self, recipe, account, *args, **kwargs):
    ...
    for s in sorted(Script.get_scripts(account.email), key=lambda s: s.get_tag()):
      ...
      self.products[s.get_product()].append(ScriptJsonForm('[BLANK]', s, {}, self.instance.get_constants(), prefix='%s_[BLANK]' % s.get_tag()))
